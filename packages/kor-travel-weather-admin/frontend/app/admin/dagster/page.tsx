...
import { DagsterRepository, DagsterRun, DagsterSchedule, DagsterSnapshot, STALLED_RUN_THRESHOLD_SECONDS, describeCron, formatElapsed, getDagsterSnapshot, isStalledRun, jobLabel, runElapsedSeconds } from "@/lib/dagster";
        ...
        <tr className="sync-run-detail-row">
          <td colSpan={3}>
            <div className="sync-run-detail-grid">
              <div><span>실행되는 job</span><code>{schedule.jobName}</code></div>
              <div><span>스케줄 이름</span><code>{schedule.name}</code></div>
              <div><span>cron</span><code>{schedule.cron ?? "—"}</code></div>
            </div>
          </td>
        </tr>
      ) : null}
  ...
  return (
    <>
      ...
      <section className="panel dagster-runs">
        ...
        {snapshot?.runs.length ? <div className="table-wrap"><table><thead><tr><th scope="col">status</th><th scope="col">job</th><th scope="col">started</th><th scope="col">finished</th><th scope="col" /></tr></thead><tbody>{snapshot.runs.map((run) => <RunRow key={run.runId} run={run} nowSeconds={nowSeconds} />)}</tbody></table></div> : <div className="empty">{snapshot ? "최근 Dagster 실행이 없습니다." : "실행 기록을 불러오는 중…"}</div>}
      </section>
      <section className="panel">
        ...
        {schedules.length ? (
          <div className="table-wrap">
            <table>
              <thead><tr><th scope="col">job</th><th scope="col">주기</th><th scope="col">상태</th></tr></thead>
              <tbody>
