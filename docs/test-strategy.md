...
- repository: PostgreSQL timezone/FK/trigger, immutable replay/revision, migration
...
root smoke는 PostgreSQL test database를 띄운 뒤
`TMPDIR=/tmp uv run --extra dev --extra dagster pytest -q`로 실행한다.
